...
Dim rs_urun : Set rs_urun = db.Execute( _
  "SELECT u.id, u.urunadi, u.akno, u.aktif_degil, u.ord, u.one_cikan, a.anakatadi, " & _
...
<div class="content">
  ...
  <table class="tbl">
    ...
    Do While Not rs_urun.EOF
      ...
      Dim u_aksiyon : u_aksiyon = rs_urun("one_cikan") : If IsNull(u_aksiyon) Then u_aksiyon=0
    ...
    %>
      <tr draggable="true" data-id="<%=u_id%>" data-ord="<%=u_ord%>" data-akno="<%=u_akno%>">
        ...
        <td>
          <% If (u_aksiyon <> 0 And Not IsNull(u_aksiyon)) Then %>
            <span style="background:#e67e22;color:#fff;border-radius:4px;padding:1px 6px;font-size:10px;font-weight:700;margin-right:5px;vertical-align:middle">
              <i class="fa fa-tag"></i> KAMPANYA
            </span>
          <% End If %>
          <strong><%=Server.HTMLEncode(u_adi)%></strong>
        </td>
        <td style="text-align:center">
          <% If u_kap > 0 Then %>
